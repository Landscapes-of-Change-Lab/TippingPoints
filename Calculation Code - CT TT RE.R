# CORRECTED Unified Transition Analysis
# FIXES: Sustained decline detection now respects alt_state_method parameter
# Date: October 2025
...
marine.data<-readRDS("C:/Users/rfidler/Documents/marine.data.raw.complete.rds")
terrestrial.data.all<-readRDS("C:/Users/rfidler/Documents/terrestrial.data.raw.complete.rds")
...
test.dat <- all.dat %>%
  ...
  mutate(
    lagcheck = case_when(
      ecosystem %in% c("Forests", "Grassland") & repdiff < 10 ~ "short",
      !ecosystem %in% c("Forests", "Grassland") & repdiff < 10 ~ "short", 
      TRUE ~ "keep"
    )
  ) %>%
  ...
  # Handle Grassland site IDs (add data source to make unique)
  mutate(std_id = case_when(
    ecosystem == "Grassland" ~ paste0(std_id, "_x_", data),
    TRUE ~ std_id
  )) %>%
...
unified_results$ecosystem_summary
